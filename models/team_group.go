...
type RUserTeamGroup struct {
	Id        string        `json:"id" grom:"column:id"`
	Name      string        `json:"name" gorm:"column:"name"`
	Username  string        `json:"username" gorm:"column:username"`
	Role      int           `json:"role" gorm:"column:role"`
	CreatedAt LocalTime     `json:"created_at" gorm:"column:created_at" type:"datetime"`
	GroupList []RGoodsGroup `json:"group_list" gorm:"column:-"`
}

// 获取用户的团队空间
func GetTeamGroupList(userId int64) []RUserTeamGroup {
	var result []RUserTeamGroup
	sqlStr := `
	SELECT tg.id,
        tg.name,
...
// 更新团队空间
func UpdateTeamGroup(teamId int64, name string) {
	sqlStr := `update team_group set name=@name where id=@teamId`
	DB.Exec(sqlStr, map[string]interface{}{"teamId": teamId, "name": name})
}
